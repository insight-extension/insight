...
const YOUR_PUBLIC_KEY = process.env.YOUR_PUBLIC_KEY || '9JxVGS9skLMRcgq5tdaGsRZSxSdAXvHkCyr2ZFTkHzbw';

// Check for required API keys
if (!SPEECHMATICS_API_KEY) {
    console.error('Speechmatics API key is not set in the environment variables.');
    process.exit(1);
}
...
function extractSentences(buffer) {
    ...
    if (sentences.length > 0) {
        ...
        const remainingBuffer = buffer.slice(lastSentenceEnd + sentences[sentences.length - 1].length);
...
app.post('/api/get-public-key', (req, res) => {
    ...
    if (session && (Date.now() - session.timestamp) < 5 * 60 * 1000) { // Valid for 5 minutes
    ...
    try {
        ...
        if (transaction && transaction.meta && transaction.meta.err === null) {
            ...
            if (isPaymentValid) {
                ...
                res.status(400).json({ error: 'Transaction does not include a transfer to your address' });
            }
        } else {
...
async function chargeUser(publicKey, amount) {
    try {
        ...
        if (user) {
            ...
            await user.save();
            console.log(`Charged ${amount} from user ${publicKey}. New balance: ${user.balance}`);
        } else {
...
wss.on('connection', async (wsClient) => {
        ...
        if (typeof data === 'string') {
            ...
            if (message.type === 'authentication') {
                ...
                if (user && user.balance > 0) {
                    ...
                    if (!assignedKey) {
                        console.error('No available Speechmatics API keys to handle a new connection.');
                        wsClient.send(JSON.stringify({ type: 'error', message: 'Server is busy. Please try again later.' }));
                        wsClient.close();
                        return;
                    }
                    ...
                    session.addListener('RecognitionStarted', () => {
                        ...
                        wsClient.send(JSON.stringify({ type: 'status', message: 'RecognitionStarted' }));
                    });
                    ...
                    session.addListener('AddTranscript', async (message) => {
                        ...
                        const { completeSentences, remainingBuffer } = extractSentences(transcriptBuffer);

                        if (completeSentences) {
                            wsClient.send(JSON.stringify({ type: 'translation_status', message: 'Translating...' }));

                            try {
                                const translatedText = await translateTextStream(completeSentences.trim(), sourceLang, targetLang);
                                console.log('Translation:', translatedText);
                                wsClient.send(JSON.stringify({ type: 'translation', translation: translatedText }));

                                wsClient.send(JSON.stringify({ type: 'translation_status', message: 'Translation complete' }));
                                ...
                                wsClient.send(JSON.stringify({ type: 'error', message: 'Failed to translate transcript' }));
                            }
                        }
                    });
                    ...
                    session.addListener('EndOfTranscript', async () => {
                        ...
                        wsClient.send(JSON.stringify({ type: 'status', message: 'EndOfTranscript' }));
                        ...
                        if (transcriptBuffer.trim().length > 0) {
                            ...
                            try {
                                const translatedText = await translateTextStream(transcriptBuffer.trim(), sourceLang, targetLang);
                                console.log('Translation:', translatedText);
                                wsClient.send(JSON.stringify({ type: 'translation', translation: translatedText }));
                    ...
                    try {
                        ...
                        wsClient.send(JSON.stringify({ type: 'error', message: 'Failed to start transcription session' }));
                        wsClient.close();
                    ...
                    wsClient.send(JSON.stringify({ type: 'error', message: 'Insufficient balance or user not found' }));
                    wsClient.close();
                }
            } else if (message.type === 'language_settings') {
                sourceLang = message.sourceLanguage || 'en';
                targetLang = message.targetLanguage || 'ua';
                console.log(`Languages set: source - ${sourceLang}, target - ${targetLang}`);
            } else {
            ...
            if (wsClient.session) {
                try {
                    ...
                    wsClient.send(JSON.stringify({ type: 'error', message: 'Failed to send audio data' }));
                }
            }
        }
    });
    ...
    wsClient.on('close', async () => {
        ...
        if (wsClient.session) {
            ...
            await wsClient.session.stop().then(() => {
    ...
    wsClient.on('error', async (error) => {
        ...
        if (wsClient.session) {
            ...
            await wsClient.session.stop().then(() => {
                ...
                console.error('Error stopping transcription session after error:', err);
                // Release API key even in case of error
                releaseSpeechmaticsKey(assignedKey);
            });
        }
